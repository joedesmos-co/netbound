...
func _test_registry() -> bool:
	...
	var passed: bool = bool(validation.ok) and all.size() == 38
...
func _test_save_selection_and_migration() -> bool:
	...
	passed = migrated.get_save_data().save_version == 2 and passed
...
func _test_unlock_conditions() -> bool:
	...
	passed = all_unlocked.get_unlocked_cosmetics().size() == 38 and passed
	local_service.free()
	all_unlocked.free()
...
func _test_cosmetics_screen() -> bool:
	...
	var passed := app.current_screen_name == "cosmetics" \
		...
		and app.cosmetic_card_buttons.size() == 18
	...
	await process_frame
	passed = app.cosmetic_card_buttons.size() == 12 and passed
	app._select_cosmetic_category("goal_effect")
	await process_frame
	passed = app.cosmetic_card_buttons.size() == 8 and passed
