...
func _test_registry() -> bool:
	...
	var passed: bool = bool(validation.ok) \
		and LevelRegistryScript.get_entries().size() == 30 \
		and LevelRegistryScript.get_level_ids()[0] == "level_01" \
		and LevelRegistryScript.get_level_ids()[-1] == "level_30" \
		...
		and LevelRegistryScript.get_next_level_id("level_20") == "level_21" \
		and LevelRegistryScript.get_next_level_id("level_29") == "level_30" \
		and LevelRegistryScript.get_next_level_id("level_30").is_empty()
	for level_id in LevelRegistryScript.get_level_ids():
...
func _test_recording() -> bool:
	...
	passed = service.is_level_completed("level_30") and passed
	passed = service.is_level_unlocked("level_21") and passed
	passed = service.get_save_data().progression.unlocked_levels.size() <= 30 and passed
	service.free()
